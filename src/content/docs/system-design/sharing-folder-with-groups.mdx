...
5. The user specifies the access type (e.g., read, owner) for the selected group.
...
13. The client sends the encrypted credential fields, user IDs, group ID, and folder ID to the server.
``` json
{
    "folderId": "folder_id",
    "groupData": [{
...
```
14. The server processes the sharing request.
15. The server adds access to the `folder_access` table for the shared folder and group ID.
16. The server adds access to the `access_list` table for each credential in the folder and the group ID.
17. The server adds the encrypted credential fields to the `fields` table for each group member.
18. The server returns the sharing response (success/failure) to the user.
